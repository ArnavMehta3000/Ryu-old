...
namespace Ryu::Gfx
{
	...
	Device::Device(HWND window, const DeviceConfig& config)
		: m_config(config)
		, m_window(window)
		, m_isWarpDevice(config.UseWARP)
	{
		RYU_PROFILE_SCOPE();
		...
		DebugLayer::Initialize(m_config.EnableDebugLayer, m_config.EnableGPUValidation);
